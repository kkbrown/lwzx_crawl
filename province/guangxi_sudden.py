    ...
    try:
        ...
        for item in data:
            try:
                ...
                dt_publish = datetime.strptime(publish_time, "%Y-%m-%d %H:%M:%S") if publish_time else None
                dt_start = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S") if start_time else None

                # 判断事件种类
                if dt_publish and dt_start and dt_publish < dt_start:
                ...
                new_item = {
                    ...
                    "event_type_name": classify_event_type(item.get("blockReason")),
                }
        ...
        logging.info(f"广西采集突发类事件完成：成功 {len(valid_data)} 条，失败 {len(error_log)} 条")
        save_to_file(valid_data, "guangxi_sudden")
