...
async def fetch_guizhou_event_data():
    filter_chain = FilterChain([
       ...
       # ),
        ...
        URLPatternFilter(patterns=["*zwgk/zdlygk/jjgzlfz/jtys/lqxx/gxgllk/*"]),
...
#        target_elements=[".detailMain"],
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        results = await crawler.arun(
            url="https://www.guizhou.gov.cn/zwgk/zdlygk/jjgzlfz/jtys/lqxx/gxgllk/",
            config=config
        )
...
async def run_task():
    try:
        logging.info("开始爬取贵州省高速（干线）路况信息...")
