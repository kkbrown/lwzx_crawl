...
async def fetch_guangdong_event_data():
    # 1. Define a simple extraction schema
    schema = {
        "name": "Guangdong_Traffic_Simple",
        "baseSelector": "ul.pcontent > li",
        "fields": [
            {
                "name": "timestamp",
                "selector": "span",
    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        result = await crawler.arun(
            url="http://www.gaosu.com.cn/lukuang/guangdong/",
...
def clean_content(content):
    """Remove leading timestamp in the format 'YYYY-MM-DD HH:MM'."""
    return re.sub(r'^\d{4}-\d{2}-\d{2} \d{2}:\d{2}', '', content)

async def run_task():
    try:
        logging.info("开始爬取广东省高速路况信息...")
        ...
        while retry_count < 100:
            try:
                data = await fetch_guangdong_event_data()
        ...
        logging.info(f"成功采集广东高速路况：{len(valid_data)} 条，异常：{len(error_log)} 条")
        save_to_file(valid_data, "guangdong")
